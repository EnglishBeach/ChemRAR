...
Метрика показывает что молекула просто синтезируется, о чем свидетельствует небольшое дерево синтеза в 1 этап. В молекуле нет сложных гетероциклов, и сама реакция понятна.

Да и сама молекула выглядит как пример из учебника по органической химии раздела конденсации ароматики.

### Сложное

**Score=0.294270595878595**

![alt text](images/image6.png)

Требуется многостадийный синтез для того чтобы построить необычный гетероцикл. Да и сама молекула достаточно сложная, что требует дополнительной стадии (р. Сузуки, если не ошибаюсь) присоединения хвоста.
...
Сложная молекула (видимо в базе не много производных декалина).Из-за обилия мест для потенциального галлогенирования сложно синтезировать из простых прекурсоров. Поэтому требуется сразу специфически замещенных прекурсор.

К тому же в базовом стоке нет воды, некоторых спиртов (этиловый, метиловый) и др простых соединений. Что требудет абсурдных стадий для их получения. (В чем можно убедиться прогнав базовый ретросинтез для ацетона, например)
...
Метрика показывает что данная молекула, имеет сложность немного выше средней и требуется модификация подхода ретросинтеза. Основная сложность проистекает из множества гетероциклов
